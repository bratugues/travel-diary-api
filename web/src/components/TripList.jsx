...
export const TripList = ({trips, onDelete, onFavorite}) => {
  return (
      ...
      {trips.map(trip => (
        <div key={trip.id} className="relative group">
           <Link key={trip.id} to={`/trips/${trip.id}`} className="block group">
              ...
              <button onClick={(e) => onDelete(e, trip.id)} className="absolute top-1 right-1 text-black-300 hover:text-red-500 transition z-10 p-2" title="Delete Trip">ⅹ</button>
